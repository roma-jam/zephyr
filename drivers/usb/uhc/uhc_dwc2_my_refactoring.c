...
struct uhc_dwc2_data {
	...
	struct {
		...
		enum uhc_dwc2_port_state port_state;
...
static inline void dwc2_port_set_power(struct usb_dwc2_reg *const dwc2,
	bool power_on)
{
	uint32_t hprt = sys_read32((mem_addr_t)&dwc2->hprt);
...
static inline int uhc_dwc2_get_port_speed(const struct device *dev, uhc_dwc2_speed_t *speed)
{
	...
	if (priv->dynamic.port_state != UHC_DWC2_PORT_STATE_ENABLED) {
...
static int uhc_dwc2_power_on(const struct device *dev)
{
	...
	if (priv->dynamic.port_state != UHC_DWC2_PORT_STATE_POWERED_OFF) {
		LOG_ERR("Port not in power off state");
		return -EPERM;
	}

	priv->dynamic.port_state = UHC_DWC2_PORT_STATE_DISCONNECTED;
...
/*
 * Initialization sequence for GUSBCFG register
 *
...
static int uhc_dwc2_init_host(const struct device *dev)
{
	...
	priv->dynamic.port_state = UHC_DWC2_PORT_STATE_POWERED_OFF;
	priv->dynamic.last_event = UHC_DWC2_PORT_EVENT_NONE;
...
static enum uhc_dwc2_port_event uhc_dwc2_decode_hprt(const struct device *dev,
	 uhc_dwc2_core_event_t core_event)
{
	...
	switch (core_event) {
		...
		case UHC_DWC2_CORE_EVENT_DISCONN: {
			/* TODO: priv->dynamic.port_state = UHC_DWC2_PORT_STATE_RECOVERY */
		...
		case UHC_DWC2_CORE_EVENT_DISABLED: {
			...
			if (priv->dynamic.port_state != UHC_DWC2_PORT_STATE_RESETTING) {
				if (priv->dynamic.flags.waiting_disable) {
					/* Disabled by request (i.e. by port command). Generate an internal event */
					priv->dynamic.port_state = UHC_DWC2_PORT_STATE_DISABLED;
					...
					LOG_ERR("Port disabled due to an error.");
					port_event = UHC_DWC2_PORT_EVENT_ERROR;
				}
			}
			break;
		}
	...
	if (core_event == UHC_DWC2_CORE_EVENT_CHAN) {
		...
		while (chan_obj != NULL) {
			...
			if (pipe_event != PIPE_EVENT_NONE) {
				...
				k_event_post(&priv->event, BIT(UHC_DWC2_EVENT_PIPE));
			}
		...
		if (core_event != UHC_DWC2_CORE_EVENT_NONE) {
			/* Port event */
			enum uhc_dwc2_port_event port_event = uhc_dwc2_decode_hprt(dev, core_event);
			if (port_event != UHC_DWC2_PORT_EVENT_NONE) {
...
static inline enum uhc_dwc2_port_event uhc_dwc2_get_port_event(const struct device *dev)
{
	...
	if (priv->dynamic.flags.event_pending) {
		...
		event = UHC_DWC2_PORT_EVENT_NONE;
	}
	/* TODO: exit critical section */
	return event;
}
...
static inline int uhc_dwc2_port_reset(const struct device *dev)
{
	...
	* states.
	* priv->dynamic.port_state == UHC_DWC2_PORT_STATE_ENABLED;
	* priv->dynamic.port_state == UHC_DWC2_PORT_STATE_DISABLED;
	...
	*/
	...
	/* TODO: enter critical section */
	if (priv->dynamic.port_state != UHC_DWC2_PORT_STATE_RESETTING || !priv->dynamic.flags.conn_dev_ena) {
		/* The port state has unexpectedly changed */
		LOG_ERR("Port state changed during reset");
		ret = -EIO;
		goto bailout;
...
static inline int uhc_dwc2_port_soft_reset(const struct device *dev)
{
	const struct uhc_dwc2_config *const config = dev->config;
	struct uhc_dwc2_data *priv = uhc_get_private(dev);
	struct usb_dwc2_reg *const dwc2 = config->base;
...
{
	struct uhc_dwc2_data *priv = uhc_get_private(dev);
	enum uhc_dwc2_port_event event = uhc_dwc2_get_port_event(dev);
	int ret;

	LOG_DBG("Port event: %s", uhc_port_event_str[event]);
	...
	switch (event) {
		...
		case UHC_DWC2_PORT_EVENT_OVERCURRENT: {
			...
			switch (priv->dynamic.port_state) {
				case UHC_DWC2_PORT_STATE_DISABLED:
					break;
				case UHC_DWC2_PORT_STATE_RESETTING:
				case UHC_DWC2_PORT_STATE_ENABLED:
...
static inline void uhc_dwc2_thread_handler(void *const arg)
{
	...
	uint32_t evt = k_event_wait(&priv->event, UINT32_MAX, false, K_FOREVER);
	...
	if (evt & BIT(UHC_DWC2_EVENT_PIPE)) {
		k_event_clear(&priv->event, BIT(UHC_DWC2_EVENT_PIPE));
				...
				pipe_hdl_t pipe_hdl, struct uhc_transfer *const xfer)
{
	...
	/* TODO: Configure the channel */
	/* TODO: Start the channel */

	/* Otherwise, fill the buffer, which is not used */
...
static int uhc_dwc2_init(const struct device *dev)
{
	...
	/* 3. Check that PHY is clocked, otherwise software reset won't finish */
	if (uhc_dwc2_quirk_is_phy_clk_off(dev)) {
		LOG_ERR("PHY clock is  turned off, cannot reset");
		return -EIO;
	}

	/* 4. Reset core after selecting PHY */
	ret = dwc2_hal_core_reset(dev);
	if (ret) {
	...
	/* 6. Init DWC2 controller as host */
	ret = uhc_dwc2_init_host(dev);
	if (ret) {
		return ret;
	}

	/* 7. Allocate memory for the channel objects */
